...
def writeToDirectory(JSONorCSV):

    if JSONorCSV == True:
        jsonout = 'CarInfoFile.json'
 
        outdir = Path.home() / 'Documents' / 'Github' / 'Group6' / 'JSONdump'
        if not Path(outdir).exists():
            Path.mkdir(outdir)

        jsonname = outdir.joinpath(outdir, jsonout)
        return jsonname
    else:
        csvout = 'CarInfoFile.csv'
 
        outdir = Path.home() / 'Documents' / 'Github' / 'Group6' / 'CSVdump'
        if not Path(outdir).exists():
            Path.mkdir(outdir)

        csvname = outdir.joinpath(outdir, csvout)
        return csvname
...
df_found_dealers.to_csv(writeToDirectory(False), index=False)
print('\nCSV Completed.')
#uses json helper function to write to python with flask folder
df_found_dealers.to_json(writeToDirectory(True), orient='table')
